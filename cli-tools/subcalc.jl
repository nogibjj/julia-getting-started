...
function main(args)
    ...
    s = ArgParseSettings(
        "Example 6 for argparse.jl: " * "commands and their associated sub-tables.",
    )
    ...
    s["jump"]["som"].description =
        "Somersault jump " *  # this is how settings are tweaked
        "mode for example 6"  # for sub-commands

    s["jump"]["clap_feet"].description =
        "Clap-feet jump " *  # notice the underscore in the name
        "mode for example 6"
